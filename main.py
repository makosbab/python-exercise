...
class Cell(object):
    ...
    def count_nbs(self, generation):
        ...
        #     print("YESH")
        north = generation[(self.offset - ROWS) % (ROWS * COLUMNS)]
        south = generation[(self.offset + ROWS) % (ROWS * COLUMNS)]
        # i + 1 - ((i mod 8) div 7) * 8
        west = generation[(self.offset + (ROWS- 1) + (ROWS * COLUMNS)) % (ROWS * COLUMNS)]
